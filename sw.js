...
// The page sends the enabled reminders list via postMessage each time the app
// opens. We schedule a setTimeout per reminder inside the SW, which survives
// backgrounding on mobile far longer than page JS does.
...
function scheduleReminders(reminders) {
  ...
  reminders.forEach(r => {
    ...
    const t = setTimeout(() => {
      ...
      // Re-schedule same reminder for same time next day
      scheduleReminders([r]);
    }, delay);
